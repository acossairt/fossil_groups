...
def plot_evol(timesteps, masses, mm_times = [[], []], thresholds = [], filename = "new_plot", bins = [], redshifts = redshifts, avg = False, normalized = False, extremum = '', quant = 0, mass_range = [], x_axis = "z_nums", fig = None, ax = None, cust_legend = [], **kwargs):
    ...
    if avg == True:
        ...
        for p in range(len(masses)): # One for each bin
            current_color = next(color)
            ax.plot(timesteps[p], masses[p], color = current_color, **kwargs)
            bin_legend_handles.append(mpatches.Patch(color=current_color, label="bin " + str(n + 1) + ": (" + "{:.2e}".format(bins[n]) + " to " + "{:.2e}".format(bins[n+1]) + ")"))
    ...
    print("bin_legend_handles: ", bin_legend_handles)
    ...
    if x_axis == "z_nums":
        ...
        ax.set_xlabel("Snapnumber (SN = 100 -> z = 0)")
    ax.set_yscale('log', nonpositive = 'clip')
    if normalized == True:
        ax.set_ylabel('Normalized Mass [M_sun/h]')
    else:
...
def NEW_plot_LMMs(LMM_times_raw, thresholds, bins, mass_range = [], filename = "new_plot", x_axis = 'z_nums', linestyle_labels = [], ax = None, **kwargs): # Note: assume LMM_times are binned
...
def NEW_plot_CDF(data_raw, comparison_data, thresholds, bins = [], redshifts = redshifts, x_axis = 'z_nums', linestyle_labels = [], ax = None, **kwargs): # data = LMM_times, comp_data = binned_masses (because that gives us the total number of trees, including those without major mergers)
...
def plot_CDF(data_raw, comparison_data, thresholds, bins = [], redshifts = redshifts, x_axis = 'z_nums'): # data = LMM_times, comp_data = binned_masses (because that gives us the total number of trees, including those without major mergers)
...
def avg_cum_maj_mergers(major_merger_times, redshifts): # Note: assume major_mergers contains multiple halos
    ...
    cum_mms = [calc_cum_maj_mergers(major_merger_times[i], redshifts) for i in range(len(major_merger_times))] # One for each halo
    ...
    #print(len(cum_mms))
    #print(len(cum_mms[0]))
    #print(range(len(cum_mms[0])))
    #all_avg = []
    #for j in range(len(cum_mms[0])):
    #    print("j is: ", j)
...
def plot_cum_mms(binned_averages, bins, thresholds, redshifts = redshifts, ax = None, linestyle_labels = []):
    # If you provide a linestyle, make sure it's in a list!
    # Current stuff
    if ax is None:
        fig, ax = plt.subplots()
    ...
    for this_thresh in thresholds:
        this_linestyle = next(linestyles)
        legs.append(Line2D([0,1],[0,1], linestyle = this_linestyle, color="black"))
        labels.append("ε > " + "{:.2e}".format(this_thresh))
...
def NEW_pdf_mms(mm_times_raw, thresholds, bins, mass_range = [], x_axis = 'z_nums', ax = None, linestyle_labels = [], **kwargs): # Note: assume mm_times_raw are binned
...
def calc_candidates_vs_threshold(fossil_group_idx, nbins):
    # Note: assume fossil_group_idx is already rearranged

    tot_num_candidates = [len(this_thresh[bin_n]) + len(this_thresh[bin_n + 1]) for bin_n in range(nbins - 1) for this_thresh in fossil_group_idx]
    binned_num_candidates = [[len(this_thresh[bin_n]) for this_thresh in fossil_group_idx] for bin_n in range(nbins)]
...
def plot_candidates_vs(thresholds, tot_num_candidates, binned_num_candidates, bins = [], fig = None, ax = None, cust_legend = [], **kwargs):
    # Finally, plot them
    if ax == None:
        fig, ax = plt.subplots()
    else:
    ...
    bin_legend_handles = []
    bin_legend_handles.append(mpatches.Patch(color= "lime", label="All candidates"))
    
    # Plot the individual bins
    for i, this_bin in enumerate(binned_num_candidates):
        current_color = next(color)
        ax.plot(thresholds, this_bin, color = current_color, label="bin " + str(i + 1), **kwargs)
        bin_legend_handles.append(mpatches.Patch(color=current_color, label="bin " + str(i + 1)))
